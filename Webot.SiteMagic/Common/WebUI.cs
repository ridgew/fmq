...
namespace System.Web
{
    /// <summary>
    /// 网页界面功能函数
    /// Author: Ridge Wong, 8th, Feb. 2006
    /// Revision: 1.0.1 2006-3-30
    /// 1.0.2 (Ridge Wong: Checked at 2006-3-31)
    /// 1.0.3 (Ridge Wong: Checked at 2006-5-17)
    /// </summary>
        ...
        public static string GeneralHtmlBind(DataTable dTab, params string[] objModel)
        {
            ...
            if (dTab != null)
            {
                for (int i = 0; i < dTab.Rows.Count; i++)
                {
                    ...
                    if (objModel.Length > 3 && objModel[3] != string.Empty && Webot.Common.Util.IsNumerical(objModel[2]))
                    {
